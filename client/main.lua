...
AddEventHandler('esx_gym:useBandage', function()
	...
	ESX.ShowNotification("Du anvönde ett ~g~bandage")
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck på ~INPUT_CONTEXT~ för att hyra en ~b~cykel')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Du har redan hyrt en ~r~cykel")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck på ~INPUT_CONTEXT~ för att öppna ~b~gym~w~ menyn')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck på ~INPUT_CONTEXT~ för att träna dina ~g~armar')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Förbreder ~g~övningen~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Du måste vila ~r~60 sekunder~w~ innan du kan göra en annan övning.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Du behöver ett medlemskap för att ~r~träna")
						end
					elseif training == true then
						ESX.ShowNotification("Du behöver vila...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck på ~INPUT_CONTEXT~ för att göra ~g~pull-ups')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck på ~INPUT_CONTEXT~ för att göra ~g~armhävningar')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Tryck på ~INPUT_CONTEXT~ för att göra lite ~g~yoga')
				...
				if IsControlJustPressed(0, Keys['E']) then
					...
					elseif training == true then
						ESX.ShowNotification("u behöver vila...")
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then	
							...
							Citizen.Wait(15000)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Du måste vila ~r~30 sekunder~w~ innan du kan göra en annan övning.")
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Du kan nu träna igen...")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = 'Öppettider', value = 'hours'},
				{label = 'Medlemskap', value = 'ship'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Vi har öppet ~g~24~w~ timmar/ dag. Välkommen!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Affär',
            elements = {
				{label = 'Protein Shake (22 SEK)', value = 'protein_shake'},
				{label = 'Water (17 SEK)', value = 'water'},
				{label = 'Sportlunch (12 SEK)', value = 'sportlunch'},
				{label = 'Powerade (25 SEK)', value = 'powerade'},
				{label = 'Bandage (50 SEK)', value = 'bandage'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Medlemskap (1000 SEK)', value = 'membership'},
            }
        },
        function(data, menu)
...
function OpenBikeMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'bike_menu',
        {
            title    = 'Hyr en cykel.',
            elements = {
				{label = 'BMX (250 SEK)', value = 'bmx'},
				{label = 'Cruiser (300 SEK)', value = 'cruiser'},
				{label = 'Fixter (329 SEK)', value = 'fixter'},
				{label = 'Scorcher (400 SEK)', value = 'scorcher'},
            }
        },
        function(data, menu)
